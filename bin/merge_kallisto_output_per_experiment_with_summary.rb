...
CSV.foreach(options[:metadata], col_sep: "\t", headers:true) do |row|
	...
	sample_id = row["Sample IDs"]
	sample_id = row["Sample.IDs"] unless sample_id
	
	id 	  	= sample_id.gsub(/\s+/,"_").gsub(",",".").gsub(":",".")

	out_d ="#{options[:output_dir]}/#{options[:ref_name]}/#{study}/#{id}"
	k+=1
	$stderr.puts "Comparing: '#{study}' '#{options[:study_title]}'"
	if options[:study_title]  and study != options[:study_title] 
		$stderr.puts "SKIP"
		next  
	end
